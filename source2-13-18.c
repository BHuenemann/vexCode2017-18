...
#include"resources\auton.h"
//#include "resources\tunes.h"

//Center right is in port 1
...
const unsigned int MG_THRESHHOLD = 2950;	//MGs are less than the threshhold and no MGs are more
const unsigned int DRIVE_THRESHOLD = 21;	//Threshold for drive motors to work
...
/* Function for linearizing motors, bounding them, and setting
*  them to a specific ratio */
int linearizeMotor(float motorSpeed, float ratio = 1, int max = 127) {
	motorSpeed = (motorSpeed > ratio * max) ? ratio * max : motorSpeed;
	motorSpeed = (motorSpeed < ratio * -max) ? ratio * -max : motorSpeed;

	int linSpeed = round(motorSpeed);
	return linSpeed;
}
...
void pidTuneClaw(int clawTarget) {
	...
	motor[claw] = linearizeMotor(drive);
}
...
void pidTuneClaw(int* clawTarget) {
	...
	*clawTarget = (*clawTarget < 0) ? 0 : *clawTarget;
	...
	datalogAddValueWithTimeStamp(1, drive);
	datalogAddValueWithTimeStamp(2, Kp_CLAW * error);
	datalogAddValueWithTimeStamp(3, Kd_CLAW * derivative);
	datalogAddValueWithTimeStamp(4, Ki_CLAW * integralClawD);
	datalogAddValueWithTimeStamp(5, *clawTarget);
	datalogAddValueWithTimeStamp(6, potToThousandths(CLAW));

	drive = (drive < -90) ? -90 : drive;

	motor[claw] = (abs(linearizeMotor(drive)) > 11) ? linearizeMotor(drive, 0.8) : 0;
}
...
*  claw. This version of the function is used during autonomous
*  in conjunction with an array */
void pidTuneMGLift(int liftMGTarget) {
	...
	motor[liftMG] = (abs(linearizeMotor(drive)) > 11) ? linearizeMotor(drive) : 0;
}

/* SEE void pidTuneMGLift(int liftMGTarget)
*  The equivalent of the above function, but the target is passed by
*  reference rather than by value. This version of the function is
*  used during driver mode in conjunction with a local variable */
void pidTuneMGLift(int* liftMGTarget) {
...
void pidTuneLift(int liftTarget) {
	...
	motor[lLift] = linearizeMotor(lDrive);
	motor[rLift] = linearizeMotor(rDrive);
}
	...
	if(!slowDown) {
		...
		*  minimum threshold otherwise */
		while(abs(SensorValue[leftEn]) < abs(enTarget) && time1[T1] < GIVE_UP_TIME) {
			...
			drive = (sgn(enTarget) > 0 && drive < DRIVE_THRESHOLD) ? DRIVE_THRESHOLD : drive;
			drive = (sgn(enTarget) < 0 && drive > -DRIVE_THRESHOLD) ? -DRIVE_THRESHOLD : drive;

			motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = linearizeMotor(drive, speedMultiplier);
			pidTuneMGLift(targets[LIFT_MG]);		//P-tunes the claw and the lifts while driving
			pidTuneLift(targets[LIFT]);
			pidTuneClaw(targets[CLAW]);
		}

		//Motors drive backward briefly in order to counteract additional drift
		motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = -sgn(enTarget) * DRIVE_THRESHOLD;
		wait1Msec(110);
		motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = 0;
	}
	/* This part is if the user doesn't want it to
	*  slow down as it reaches the target */
	else if(slowDown) {
		while(abs(SensorValue[leftEn]) < abs(enTarget) && time1[T1] < GIVE_UP_TIME) {
			motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = linearizeMotor(sgn(enTarget) * 127, speedMultiplier);
			pidTuneMGLift(targets[LIFT_MG]);		//P-tunes the claw and the lifts while driving
			pidTuneLift(targets[LIFT]);
			pidTuneClaw(targets[CLAW]);
		}
...
void waitRotate(const int* const targets, int degrees10, bool slowDown = false, float speedMultiplier = 1) {
	...
	if(!slowDown) {
		if(degrees10 > 0) {
			while(SensorValue[yaw] < degrees10 && time1(T1) < GIVE_UP_TIME) {
				...
				drive = (abs(drive) > 90) ? sgn(degrees10) * 90 : drive;
				drive = (sgn(degrees10) > 0 && drive < DRIVE_THRESHOLD) ? DRIVE_THRESHOLD : drive;
				drive = (sgn(degrees10) < 0 && drive > -DRIVE_THRESHOLD) ? -DRIVE_THRESHOLD : drive;

				motor[bR] = motor[cR] = motor[fR] = linearizeMotor(drive, speedMultiplier, 90);		//The right side moves forwards
				motor[bL] = motor[cL] = motor[fL] = -linearizeMotor(drive, speedMultiplier, 90);	//The left side moves backwards
				pidTuneMGLift(targets[LIFT_MG]);		//P-tunes the claws and the lift while rotating
				pidTuneLift(targets[LIFT]);
				pidTuneClaw(targets[CLAW]);
			}
		}
		...
		motor[bR] = motor[cR] = motor[fR] = -sgn(degrees10) * DRIVE_THRESHOLD;
		motor[bL] = motor[cL] = motor[fL] = sgn(degrees10) * DRIVE_THRESHOLD;
		wait1Msec(110);
		motor[bR]  = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = 0;
	}
	/* This part is if the user doesn't want it to
	*  slow down as it reaches the target */
		...
		if(degrees10 > 0) {
			while(SensorValue[yaw] < degrees10 && time1(T1) < GIVE_UP_TIME) {
				motor[bR] = motor[cR] = motor[fR] = linearizeMotor(90, speedMultiplier, 90);	//The right side moves forwards
				motor[bL] = motor[cL] = motor[fL] = -linearizeMotor(90, speedMultiplier, 90);	//The left side moves backwards
				pidTuneMGLift(targets[LIFT_MG]);		//P-tunes the claws and the lift while rotating
				pidTuneLift(targets[LIFT]);
				pidTuneClaw(targets[CLAW]);
			}
	...
	if(!slowDown) {
		if(outTargetX > 0) {
			while((SensorValue[leftEn] < outDistance || SensorValue[rightEn] < inDistance) && time1[T1] < GIVE_UP_TIME) {
				...
				lDrive = (lDrive < DRIVE_THRESHOLD) ? direction * DRIVE_THRESHOLD : lDrive;
				rDrive = (rDrive < DRIVE_THRESHOLD) ? direction * DRIVE_THRESHOLD : rDrive;

				motor[bR] = motor[cR] = motor[fR] = linearizeMotor(rDrive, speedMultiplier * speedRatio);
				motor[bL] = motor[cL] = motor[fL] = linearizeMotor(lDrive, speedMultiplier);
				pidTuneMGLift(targets[LIFT_MG]);
				pidTuneLift(targets[LIFT]);
				pidTuneClaw(targets[CLAW]);
			}
		}
		...
		*  but it accounds for the inner distance being on the left. */
			...
			while((SensorValue[leftEn] < inDistance || SensorValue[rightEn] < outDistance) && time1[T1] < 5000) {
				...
				motor[bR] = motor[cR] = motor[fR] = linearizeMotor(rDrive, speedMultiplier);
				motor[bL] = motor[cL] = motor[fL] = linearizeMotor(lDrive, speedMultiplier * speedRatio);
				pidTuneMGLift(targets[LIFT_MG]);
				pidTuneLift(targets[LIFT]);
				pidTuneClaw(targets[CLAW]);
			}
		...
		//Motors drive backward briefly in order to counteract additional drift
		motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = -direction * DRIVE_THRESHOLD;
		wait1Msec(110);
		motor[bR] = motor[cR] = motor[fR] = motor[bL] = motor[cL] = motor[fL] = 0;
	}
	/* This part is if the user doesn't want it to
...
int axesFix(int remote) {
	if(abs(remote) >= DRIVE_THRESHOLD) 		//Returns 0 if the value does not pass the TRESHOLD constant
		return remote;
	else
		return 0;
}
	...
	if(autonToggleState) {
		...
		//Regular Autonomous
			...
			if(autonTypeToggleState == 1) {
				...
				motor[bL] = motor[cL] = motor[fL] = motor[bR] = motor[cR] = motor[fR] = 127;
				wait1Msec(1500);
				motor[bL] = motor[cL] = motor[fL] = motor[bR] = motor[cR] = motor[fR] = 0;
				setClaw(targets, 0);
				motor[liftMG] = 127;
				wait1Msec(1000);
				motor[liftMG] = 0;
...
task usercontrol()
{
	...
	/* Variables for having the program record
	*  each motor input during a skills run */
	int prevDL = 0;
	int prevDR = 0;
	int prevLift = 0;
	int prevLiftMG = 0;
	...
	while (1)
	{
		...
		//Give up function
		if(vexRT[Btn5U] && vexRT[Btn5D] && vexRT[Btn8R]) {
			allMotorsOff();
		...
		//Button 6U and 6D take priority over autostack
			...
			if((!stackCancel) && autoStack && SensorValue(mgIn) < MG_THRESHHOLD) {
				//Rotates inward if the lift is above the stack count and registers there is a cone
				if((SensorValue(Sonar) < SONAR_CONE - SONAR_TOL || SensorValue(Sonar) > SONAR_CONE + SONAR_TOL) && potToThousandths(LIFT) > STACK_HEIGHT + 50) {
					clawPosition = -1;
				}
			}
		}
		...
		if(clawPosition == -1) {
			...
			//motor[claw] = (potToThousandths(CLAW) < 1000 - CLAW_TOL - 100) ? 127 : 0;
			clawTarget = 1000;
		}
		...
		if(vexRT[Btn5U]) {
			...
			motor[rLift] = motor[lLift] = (potToThousandths(LIFT) <= 1000) ? 127 : 0;
		...
		if(vexRT[Btn7L]) {
			...
			mgLiftTarget = (mgLiftTarget < 0) ? 0 : mgLiftTarget;
			mgLiftTarget = (mgLiftTarget > 1000) ? 1000 : mgLiftTarget;

			pidTuneMGLift(&mgLiftTarget);
		}

		/* Series of conditionals and print statements to test an idea.
		*  The idea is to record the motor movements during a driver
		*  skills run and perform those exact movements again
		*  during programming skills */
		if(prevDL != motor[bL]) {
			if(waitCount > 0) {
				writeDebugStreamLine("wait1Msec(%d);", waitCount);
				hasWaited = true;
			}
			if(!hasWaited && !waitCount) {
				writeDebugStreamLine("wait1Msec(20);");
				hasWaited = true;
			}
			writeDebugStreamLine("motor[bL] = motor[cL] = motor[fL] = %d;", motor[bL]);
			waitCount = 0;
		}
		if(prevDR != motor[bR]) {
			if(waitCount > 0) {
			...
			writeDebugStreamLine("motor[bR] = motor[cR] = motor[fR] = %d;", motor[bR]);
			waitCount = 0;
		}
		if(prevLift != motor[lLift]) {
			if(waitCount > 0) {
				writeDebugStreamLine("wait1Msec(%d);", waitCount);
				hasWaited = true;
			}
			...
			writeDebugStreamLine("motor[lLift] = motor[rLift] = %d;", motor[lLift]);
			waitCount = 0;
		}
		if(prevLiftMG != motor[liftMG]) {
			if(waitCount > 0) {
				writeDebugStreamLine("wait1Msec(%d);", waitCount);
				hasWaited = true;
			}
			...
			writeDebugStreamLine("motor[liftMG] = %d;", motor[liftMG]);
			waitCount = 0;
		}
		else
			waitCount += 20;
		...
		prevDL = motor[bL];
		prevDR = motor[bR];
		prevLift = motor[lLift];
		prevLiftMG = motor[liftMG];
