...
const unsigned int STACK_HEIGHT = 32;		//Height for lift to raise with each cone stack
...
const unsigned int R_LIFT_MAX = 3130;		//The maximum for the right lift potentiometer
const unsigned int R_LIFT_MIN = 1550;		//The minimum for the right lift potentiometer
...
const float Ki_LIFT = 0.001;
const float Kd_LIFT = 0;
...
static bool skillsToggleState = false;		//Variable to check if skills is toggled
...
task usercontrol()
{
	...
	while (1)
	{
		...
		if(vexRT[Btn6U]) {
			...
			if(potToThousandths(CLAW) < 300 - CLAW_TOL)
				motor[lClaw] = motor[rClaw] = -60;
			else if(potToThousandths(CLAW) > 300)
				motor[lClaw] = motor[rClaw] = 60;
			else
				motor[lClaw] = motor[rClaw] = 0;
		}

/*		if(potToThousandths(LIFT) > currentStackHeight + 200 && potToThousandths(CLAW) >= CLAW_TOL) {
			motor[lClaw] = motor[rClaw] = 127;
		}
		else if(potToThousandths(LIFT) < currentStackHeight + 10 && potToThousandths(CLAW) >= CLAW_TOL) {
			stackCount++;
			motor[lClaw] = motor[rClaw] = -127;
		}
		else {
...
/*		if(vexRT[Btn8D]) {
		...
		if(vexRT[Btn5U]) {
			...
			motor[rLift] = motor[lLift] = (potToThousandths(LIFT) >= 0 + LIFT_TOL) ? -127 : 0;
		}
		else {
			motor[rLift] = motor[lLift] = 0;
		}
