...
const unsigned int L_LIFT_MAX = 1950;				//The maximum for the left lift potentiometer
const unsigned int L_LIFT_MIN = 600;				//The minimum for the left lift potentiometer
const unsigned int R_LIFT_MAX = 1800;				//The maximum for the right lift potentiometer
const unsigned int R_LIFT_MIN = 600;				//The minimum for the right lift potentiometer
...
const float I_LIFT = 0.00001;
const float D_LIFT = 0.3;
...
void pidTuneLift(int liftTarget) {
	...
	lError = P_LIFT * (thousandthsToPot(LIFT, liftTarget, left) - SensorValue[lLiftPot]);
	rError = P_LIFT * (thousandthsToPot(LIFT, liftTarget, right) - SensorValue[rLiftPot]);
	lIntegral += lError;
	rIntegral += rError;
	...
	motor[lTopLift] = motor[lBotLift] = (abs(lError) >= LIFT_TOL) ?
	(P_LIFT * lError) + (I_LIFT * lIntegral) + (D_LIFT * lDerivative) : 0;
	motor[rTopLift] = motor[rBotLift] = (abs(rError) >= LIFT_TOL) ?
	(P_LIFT * rError) + (I_LIFT * rIntegral) + (D_LIFT * rDerivative): 0;
}
...
void pidTuneLift(int* liftTarget) {
	...
	lError = thousandthsToPot(LIFT, *liftTarget, left) - SensorValue[lLiftPot];
	rError = thousandthsToPot(LIFT, *liftTarget, right) - SensorValue[rLiftPot];
	lIntegral += lError;
	rIntegral += rError;
	lDerivative = lError - lPrevError;
	rDerivative = rError - rPrevError;
...
task usercontrol()
{
	...
	while (1)
	{
		...
		else if(vexRT[Btn8U] && !stackPhase) {
			motor[lBotLift] = motor[lTopLift] = motor[rBotLift] = motor[rTopLift] = 127;
			liftTarget = 1000;
		}
		else if(vexRT[Btn5U] || vexRT[Btn5D] && !stackPhase && !(T1 % 3))	//5U and 5D shoulder buttons increment and decrement the lift target
			liftTarget += (vexRT[Btn5U] - vexRT[Btn5D]);

		if(!vexRT[Btn8L])
			pidTuneLift(&liftTarget);														//P-tunes the lift
		...
		*stackCountL = 0;
		...
		case 0:
			break;
		case 1:
			liftTarget = BASE_HEIGHT + 400 + currentStackHeight;
