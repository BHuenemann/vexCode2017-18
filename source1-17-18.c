...
//Global variable used for stack count
...
void setClaw(int clawTarget) {
	if(clawTarget < potToThousandths(CLAW) - CLAW_TOL)
		motor[lClaw] = motor[rClaw] = (SensorValue[clawPot] >= CLAW_MIN + CLAW_TOL) ? 127 : 0;
	else if(clawTarget > potToThousandths(CLAW) + CLAW_TOL)
		motor[lClaw] = motor[rClaw] = (SensorValue[clawPot] <= CLAW_MAX - CLAW_TOL) ? -127 : 0;
	else
...
void stackCone(int* targets) {
	int currentStackHeight = stackCountG * STACK_HEIGHT;			//Create a variable for the current height of the cone stack
...
task autonomous()
{
	...
	*  values for the mglift, claw, rotation, and lift */
	...
	//Regular Autonomous
		...
		setMGLift(targets, 0);
		setLift(targets, BASE_HEIGHT, 100);
		setClaw(targets, 1000);
		setLift(targets, 200, 100);
		setClaw(targets, 0);
		waitDistance(targets, -400);
		waitRotate(targets, 1700);
		waitDistance(targets, 1600);
		waitRotate(targets, 400);
...
task usercontrol()
{
	...
	int currentStackHeight = stackCountG * STACK_HEIGHT; //Creates a variable for the height to stack at
	...
	while (1)
	{
		...
		*  See void stackCone(int* targets) */
		if(vexRT[Btn8U]) {
			currentStackHeight = stackCountG * STACK_HEIGHT; //Creates a variable for the height to stack at
...
//				writeDebugStreamLine("%d", potToThousandths(CLAW));
			}
			motor[lLift] = motor[rLift] = 0;
			motor[lClaw] = motor[rClaw] = 0;

			liftTarget = potToThousandths(LIFT);	//Resets lift target
			stackCountG++;												//Increments stack count
		}
