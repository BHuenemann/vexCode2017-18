...
#pragma config(Sensor, in3,    rLiftPot,       sensorPotentiometer)
#pragma config(Sensor, in4,    lLiftPot,       sensorPotentiometer)
...
#pragma config(Motor,  port1,           rLiftMG,       tmotorVex393_HBridge, openLoop)
#pragma config(Motor,  port2,           lLiftMG,       tmotorVex393_MC29, openLoop, reversed)
...
#pragma config(Motor,  port5,           rClaw,         tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port6,           lClaw,         tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port7,           fR,            tmotorVex393_MC29, openLoop, reversed)
...
const unsigned int CLAW_MAX = 4000;			//The maximum for the claw potentiometer
const unsigned int CLAW_MIN = 2000;			//The minimum for the claw potentiometer
...
bool isStackingG = false;						//Variable to keep track of if it's stacking a cone
...
float thousandthsToPot(targetTypes type, int thousandths, tDirections side = left) {
	...
	((type == CLAW) ? CLAW_MAX : ((side == left) ? L_LIFT_MAX : R_LIFT_MAX));
	float min = (type == LIFT_MG) ? LIFT_MG_MIN :
	((type == CLAW) ? CLAW_MIN : ((side == left) ? L_LIFT_MIN : R_LIFT_MIN));
...
void pTuneMGLift(int liftMGTarget) {
	...
	//Finds the error mutplied by the p-value of the mobile goal lift
	int liftMGError = P_LIFT_MG * (thousandthsToPot(LIFT_MG, liftMGTarget, left) - SensorValue[mgPot]);
...
void pTuneLift(int liftTarget) {
	...
	motor[lLift] = ((abs(lError) >= LIFT_TOL) && (abs(lError) > 20)) ? lError : 0;
	motor[rLift] = ((abs(rError) >= LIFT_TOL) && (abs(rError) > 20)) ? rError : 0;
}
...
void pidTuneLift(int* liftTarget, int* lIntegral, int* rIntegral, int* lPrevError, int* rPrevError) {
	...
	*rPrevError = rError;
	...
	motor[lLift] = ((abs(lError) >= LIFT_TOL) && (abs(lDrive) > 30)) ? lDrive : 0;
	motor[rLift] = ((abs(rError) >= LIFT_TOL) && (abs(rDrive) > 30)) ? rDrive : 0;
}
...
task stackCone() {
	bool* isStackingL = &isStackingG;	//Creates a local variable for telling if it's stacking
	...
	*isStackingL = true;		//Sets the local stacking variable equal to true
	...
	*isStackingL = false;		//Resets the local stacking variable to false
}
...
task usercontrol()
{
	...
	bool stackReset = false;					//Variable to keep track of if the stacking task has been reset

	bool* isStackingL = &isStackingG;
	...
	while (1)
	{
		...
		//Claw input:
		if(vexRT[Btn6U] || vexRT[Btn6D] && !(*isStackingL)) {
			//6U and 6D shoulder buttons increment and decrement the claw target
			clawTarget += (vexRT[Btn5U] - vexRT[Btn5D]);
		}

		pTuneClaw(clawTarget);

		//Lift input:
		if(vexRT[Btn8D] && !(*isStackingL) && !(vexRT[Btn5U] && vexRT[Btn5D])) {
			//This button tells the motors to run at full speed downwards to help pick up cones
			motor[lLift] = motor[rLift] = -127;
			liftTarget = potToThousandths(LIFT);
		}
		else if(vexRT[Btn8L] && !(*isStackingL) && !(vexRT[Btn5U] && vexRT[Btn5D])) {
			//This button tells the motors to run at full speed upwards to help it get over a cone stack
			motor[lLift] = motor[rLift] = 127;
			liftTarget = potToThousandths(LIFT);
		}
		else if(vexRT[Btn5U] || vexRT[Btn5D] && !(*isStackingL) && !(T1 % 3)) {
		...
		if(vexRT[Btn7L] || vexRT[Btn7D] && !(*isStackingL))		//Raises the lift if 7L or 7d is pressed
		...
		*  See void stackCone(int* targets) */
		if(vexRT[Btn8U] && !(*isStackingL)) {
			startTask(stackCone);
			clearTimer(T2);
			stackReset = false;
		}
		else if(vexRT[Btn8R] && !(*isStackingL)) {		//Resets the stack count if 8L is pressed
			stackCountG = 0;
			playTone(525, 1000);
		}
		if(time1[T2] > 10000 && *isStackingL && !stackReset) {
			stopTask(stackCone);
			*isStackingL = false;
			stackReset = true;
		}
		wait1Msec(20);
		//This waits 20ms between each loop since that's the update speed of the VEX cortex
	}
}
